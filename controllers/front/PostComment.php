...
class ProductCommentsPostCommentModuleFrontController extends ModuleFrontController
{
    public function display()
    {
        ...
        $this->addCommentGrades($productComment);
     ...
     * @throws Exception
     */
    private function addCommentGrades(ProductComment $productComment)
    {
        /** @var EntityManagerInterface $entityManager */
        $entityManager = $this->container->get('doctrine.orm.entity_manager');
        $averageGrade = 0;

            $productGrade = new ProductCommentGrade(
                $productComment,
                $grade
            );

            $entityManager->persist($productGrade);
            $averageGrade += $grade;

        $averageGrade /= count($productComment);
